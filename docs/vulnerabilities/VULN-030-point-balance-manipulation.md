...
2026-09-01, 로컬 재현(`:8090`, `user1`): `pointsUsed=999999` 주문 생성 → 응답 `amount=-980999`(결제
예상금액 음수, 생성 시점에 재현). 단 **잔액 차감은 결제 확인(confirm) 시점으로 이동**했으므로, 주문
생성 직후 `GET /api/points`는 잔액이 그대로다(120100→120100). 실제 "잔액 음수"까지 가려면 confirm이
성공해야 하는데 confirm은 `TOSS_SECRET_KEY` 없으면 501 → **무-Toss 랩에서는 잔액 드레인 미재현**.
확인되는 것은 생성 시 결제액 조작(음수 total)이며, 잔액 차감 조작은 Toss 결제 완주가 전제.
