...
- 심각도: Medium
- 분류: A01:2025 Broken Access Control (IDOR, CWE-639)
...
"내 위시리스트" 조회가 `userId` 쿼리 파라미터를 받는 설계 자체가 다소 어색하다(세션 사용자로 결정하면
그만) — 그 파라미터 신뢰가 곧 IDOR sink다. 정상 구현은 세션 사용자만 사용하고 파라미터를 받지 않는다.
