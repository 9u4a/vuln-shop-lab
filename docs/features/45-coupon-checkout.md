...
- **결제 미완료 시 포인트·쿠폰 사용처리 버그 수정**: 포인트 차감/적립과 쿠폰 `used` 마킹을
  주문 생성 시점에서 **결제 확인(`POST /api/orders/:id/confirm`) 성공 시점으로 이동**. 결제창에
  진입만 하고 완료하지 않으면 포인트·쿠폰이 소모되지 않는다(주문은 `pending`으로 남음).
  `orders.points_used` 컬럼 추가, confirm에 재적용 방지(`status='paid'`) 가드. 할인액·결제 예상금액
  계산은 생성 시점 그대로라 VULN-030/036의 무검증 결함은 유지(관련 문서 반영).
