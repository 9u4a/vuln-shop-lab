...
- (기능 58 배치, 2026-09-02) 토큰 인코딩을 `base64(orderId)` → `base64url(urlencode("oid="+id))`로 강화.
  겉보기만 복잡할 뿐 여전히 예측·위조·역산 가능 = VULN-038 성질 유지. 인코딩을 node `share-token.js` /
  java `ShareTokens`로 단일화(백필도 전 주문 재계산). 공유/주문상세 배송 표시는 이벤트 타임라인(기능 59)으로 확장.
