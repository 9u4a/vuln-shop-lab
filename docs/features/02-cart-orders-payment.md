...
- (기능 58 배치, 2026-09-02) 영수증을 한 줄 `echo` → 항목·합계·일자·수령인·배송지를 담은 다행 텍스트/HTML로 충실화, 주문 상세 영수증 UI를 단일 카드로 정리. **취약점 싱크는 그대로 유지**: `note`·수령인명이 셸 문자열에 보간(VULN-010/019), print HTML에 `note` raw 삽입(VULN-016), 다운로드 `filename` 미검증(VULN-011).
