...
module.exports = {
  '/orders': {
    ...
    post: {
      tags: TAG,
      summary: '주문 생성',
      description: '가격은 서버가 계산한다. `pointsUsed` 는 검증 없이 차감된다.',
      security: AUTH,
      requestBody: {
        required: true,
        content: {
          'application/json': {
            ...
            example: { items: [{ productId: 1, quantity: 1 }] },
          },
        },
      },
      responses: {
  ...
  '/orders/{id}': {
    get: {
      ...
      parameters: idParam,
      responses: {
        200: jsonOk({ type: 'object', properties: { order: ref('Order'), items: { type: 'array', items: ref('OrderItem') } } }),
        401: rref('Unauthorized'),
        404: rref('NotFound'),
      },
    },
